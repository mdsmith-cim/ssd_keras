...
def _greedy_nms(predictions, class_id, iou_threshold=0.45, coords='corners', border_pixels='half'):
    '''
    The same greedy non-maximum suppression algorithm as above, but slightly modified for use as an internal
    function for per-class NMS in `decode_detections().  Modified to work with a class_id instead of assuming a length 5 second axis with confidence the first value.`.
    '''
    boxes_left = np.copy(predictions)
    maxima = [] # This is where we store the boxes that make it through the non-maximum suppression
    while boxes_left.shape[0] > 0: # While there are still boxes left to compare...
        maximum_index = np.argmax(boxes_left[:, class_id]) # ...get the index of the next box with the highest confidence...
        maximum_box = np.copy(boxes_left[maximum_index]) # ...copy that box and...
        maxima.append(maximum_box) # ...append it to `maxima` because we'll definitely keep it
        boxes_left = np.delete(boxes_left, maximum_index, axis=0) # Now remove the maximum box from `boxes_left`
        if boxes_left.shape[0] == 0: break # If there are no boxes left after this step, break. Otherwise...
        similarities = iou(boxes_left[:, -4:], maximum_box[-4:], coords=coords, mode='element-wise', border_pixels=border_pixels) # ...compare (IoU) the other left over boxes to the maximum box...
        boxes_left = boxes_left[similarities <= iou_threshold] # ...so that we can remove the ones that overlap too much with the maximum box
...
def decode_detections(y_pred,
    ...
    Returns:
        ...
        for one image and contains a Numpy array of shape `(boxes, num_classes + 6)` where each row is a box prediction for
        a non-background class for the respective image in the format `[class_id, confidence, n_classes softmax, xmin, ymin, xmax, ymax]`.
    '''
    ...
    for batch_item in y_pred_decoded_raw: # `batch_item` has shape `[n_boxes, n_classes + 4 coords]`
        ...
        for class_id in range(1, n_classes): # For each class except the background class (which has class ID 0)...
            ...
            #Previously we indexed to keep only the single class, but we need to preserve the values for all even as we only consider one class at a time
            threshold_met = batch_item[batch_item[:, class_id] > confidence_thresh] # ...keep only those boxes with a confidence *for our class of interest* above the set threshold.
            if threshold_met.shape[0] > 0: # If any boxes made the threshold i.e. num_boxes > 0
                maxima = _greedy_nms(threshold_met, class_id, iou_threshold=iou_threshold, coords='corners', border_pixels=border_pixels) # ...perform NMS on them.
                maxima_output = np.zeros((maxima.shape[0], maxima.shape[1] + 2)) # Expand the last dimension by two elements to have room for the class ID and relevant confidence. This is now an arrray of shape `[n_boxes, n_classes + 6]`
                maxima_output[:, 0] = class_id # Write the class ID to the first column...
                maxima_output[:, 1] = maxima[:, class_id] #..confidence for that class ID to the second column (for ease of sorting later)
                maxima_output[:, 2:] = maxima # ...and write the maxima to the other columns...
                # Final order is class_id + confidence + n_classes softmax + boxes
